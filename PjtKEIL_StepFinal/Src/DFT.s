...
DFT_ModuleAuCarre proc
	...
	STR R0, [R2] ; On stocke l'adresse du premier élément du tableau d'échantillons dans IndexTable
	LDR R2, [R2]
	MOV R0, #0
	LDR R3,=TabCos ; première adresse du tableau cos
	LDR R5,=TabSin
	MOV R4, #0 ; n
	
Continue_DFT_1
	
	MUL R9, R1, R4 ; p = k*n avec ce p on va accéder à la bonne case du TabCos
	AND R9, #0x3F
	LDRSH R10, [R2] ; x(n)

	; Partie REELLE
	LDRSH R7, [R3,R9,LSL#1] ; On accède à la valeur du cos(p)
	MUL R8, R10, R7 ; On calcule la partie réelle
	ADD R0,R8 ; Sortie de la fonction : résultat de la somme
	
	; Partie IMAGINAIRE
	LDRSH R7, [R5,R9,LSL#1] ; On accède à la valeur du sin(p)
	MUL R8, R10, R7 ; On calcule la partie imaginaire
	ADD R6,R8 ; Sortie de la fonction : résultat de la somme
	
	ADD R2, #2 ; on décale de 16 bits pour pointer sur la prochaine adresse du tableau des échantillons
	ADD R4, #1 ; on incrémente n de 1
	CMP R4, #64
	BNE Continue_DFT_1
	
	; Partie 3
	ASR R0,#16
	MUL R0,R0 ; Résultat en 10.22
	
	ASR R6,#16
	MUL R6,R6 ; Résultat en 10.22
